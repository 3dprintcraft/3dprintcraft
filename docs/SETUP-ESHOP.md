...
- **Νέο προϊόν / αλλαγή τιμής** → δες την ενότητα «Πώς προσθέτω προϊόν»
  παρακάτω.
...
Όλα τα προϊόντα ζουν στο `shop/products.json`, στη λίστα `products`. Κάθε
προϊόν είναι ένα μπλοκ — αντιγράφεις το template, το προσαρμόζεις, commit,
push, τέλος. Δεν αγγίζεις ποτέ κώδικα.

**Βήματα:**

1. Βάλε τη φωτογραφία στο `shop/assets/img/` — τετράγωνη, ~800×800px,
   webp ή jpg, με ΝΕΟ όνομα αρχείου (π.χ. `magnet-logo-1.webp`).
2. Αντέγραψε το template στο τέλος της λίστας `products` (πρόσεξε το κόμμα
   ανάμεσα στα μπλοκ):

   ```json
   {
     "id": "magnet-logo",
     "name": "Μαγνητάκι με Λογότυπο",
     "category": "prints",
     "price": 4.50,
     "featured": false,
     "images": ["assets/img/magnet-logo-1.webp"],
     "description": "Μία-δύο προτάσεις που πουλάνε το προϊόν.",
     "options": [
       { "type": "select", "id": "color", "label": "Χρώμα", "choices": [
         { "value": "black", "label": "Μαύρο" },
         { "value": "gold", "label": "Χρυσό", "priceDelta": 1.50 }
       ] },
       { "type": "text", "id": "text", "label": "Το κείμενό σου", "required": true, "maxLength": 30 }
     ]
   }
   ```
...
| Πεδίο | Τι κάνει |
|---|---|
| `id` | Μοναδικό, λατινικά με παύλες — μπαίνει στο URL. ΜΗΝ το αλλάξεις μετά. |
| `category` | `nfc` ή `prints` — καθορίζει το φίλτρο και το σήμα στην κάρτα |
| `price` | Τελική τιμή με ΦΠΑ, σε ευρώ |
| `featured` | `true` = εμφανίζεται πρώτο στον κατάλογο |
| `options` | Ό,τι διαλέγει ο πελάτης. `select` = κουμπιά (με προαιρετικό `priceDelta` ±€), `text` = ελεύθερο πεδίο (όνομα, link κ.λπ.) |

Προϊόν χωρίς επιλογές; Βάλε `"options": []`. Θες να το αποσύρεις; Σβήσε το
μπλοκ — ο σύνδεσμος θα δείχνει ευγενικό «Δεν βρέθηκε».

**Κλιμάκωση:** ο μηχανισμός αντέχει άνετα εκατοντάδες προϊόντα — ο κατάλογος
έχει αναζήτηση και φίλτρα, οι φωτογραφίες φορτώνουν lazy, και ο server
διαβάζει τον ίδιο κατάλογο για τις τιμές. Από ~50 προϊόντα και πάνω αξίζει
ένα τοπικό εργαλείο-φόρμα που γράφει το JSON αυτόματα αντί για χέρι (ζήτησέ
το όταν φτάσεις εκεί).

**SEO:** πρόσθεσε το νέο προϊόν και στο `sitemap.xml` (μία γραμμή
`<url><loc>...product.html?id=ΤΟ-ID</loc>...`). Μετά το go-live, δήλωσε το
site στο **Google Search Console** (δωρεάν) — εκεί υποβάλλεις το sitemap και
βλέπεις πώς σε δείχνει το Google. Οι αλλαγές σε τίτλο/περιγραφή αργούν λίγες
μέρες να φανούν στα αποτελέσματα.
